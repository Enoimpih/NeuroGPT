...
def train(config: Dict = None) -> Trainer:
    ...
    if config["training_style"] == 'decoding':
        ...
        # MotorImagery Dataset
        # train_folds, test_folds = cv_split_bci(sorted(os.listdir(downstream_path))[:18])
        # train_files = train_folds[config['fold_i']]  # filenames
        # test_files = test_folds[config['fold_i']]  # filenames
        #
        ...
        train_files = [x for x in glob.glob(os.path.join(downstream_path, "train_-_*")) if
                       os.path.basename(x).split("_-_")[-1].split(".")[0] in features]  # 508*2
        val_files = [x for x in glob.glob(os.path.join(downstream_path, "val_-_*")) if
                     os.path.basename(x).split("_-_")[-1].split(".")[0] in features]
        test_files = [x for x in glob.glob(os.path.join(downstream_path, "test_-_*")) if
                      os.path.basename(x).split("_-_")[-1].split(".")[0] in features]
